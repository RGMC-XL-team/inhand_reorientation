...
from isaacgym.torch_utils import quat_conjugate, quat_mul, quat_rotate, to_torch, \
                                unscale, quat_apply, tensor_clamp, torch_rand_float, \
                                scale, quat_from_euler_xyz, quat_unit
...
class LeapHandRot(VecTaskRot):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(num_envs):
            ...
            self.object_pose_neutral = torch.tensor([obj_pose.p.x, obj_pose.p.y, obj_pose.p.z], device=self.device).repeat(self.num_envs, 1)
    ...
    def get_obj_pose_noise(self, obj_pose):
        ...
        rand_angles = torch_rand_float(-1.0, 1.0, (torch.count_nonzero(outlier_indices).item(), 3), device=self.device) * torch.pi
    ...
    def compute_observations(self):
        ...
        if self.cfg["env"]["include_history"]:
            ...
            t_buf = (self.obs_buf_lag_history[:, -history_length:].reshape(self.num_envs, -1)).clone() # attach three timesteps of history
