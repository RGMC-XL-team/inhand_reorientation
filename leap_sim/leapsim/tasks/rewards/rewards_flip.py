...
def compute_reward(
    ...
    dof_torque, dof_pos, target_dof_pos, dof_pos_mask,
    rot_reward_scale: float, rot_eps: float,
    pos_reward_scale: float, pos_eps: float,
    dof_pos_reward_scale: float,
    reach_goal_bonus: float, fall_dist: float,
    fall_penalty: float, success_tolerance: float, success_tolerance_pos: float,
    ftip_reward_scale: float,
    energy_scale: float, dof_pos_diff_thresh: float, dof_vel_thresh: float,
    obj_lin_vel_thresh: float, obj_ang_vel_thresh: float, action_norm_thresh: float,
    penalize_palm_contact: bool, palm_cf, palm_cf_scale: float,
    ...
    goal_reach = (abs_rot_dist <= success_tolerance) \
                 ...
                 & (dof_pos_diff <= dof_pos_diff_thresh)    # this forces the fingers to reset after manipulation
    if penalize_palm_contact:
    ...
    return reward, dones.int(), resets, goal_resets, progress_buf, successes, abs_rot_dist, abs_pos_dist, reward_terms, time_due_envs
...
def compute_leaphand_reward(
    ...
    dof_torque=None, dof_pos=None, target_dof_pos=None, dof_pos_mask=None,
    palm_cf=None
):
