...
from isaacgym.torch_utils import quat_apply, quat_conjugate, quat_mul, tensor_clamp, to_torch, torch_rand_float, unscale

from leapsim.utils.torch_jit_utils import quat_from_euler_xyz, random_quaternions

from .base.vec_task import VecTaskRot


class LeapHandRotNew(VecTaskRot):
    def __init__(
        ...
        self.prob_use_reset_state_buffer = cfg["env"]["prob_use_reset_state_buffer"]
        ...
        # self.reset_state_replay_buffers = ResetBuffer(self.num_leap_hand_dofs+7, size=2048, device=self.device)
        ...
        self.default_dof_pos_ratio = torch.tensor(
            [0.5, 0.4, 0.6, 0.6, 0.4, 0.65, 0.8, 0.85, 0.4, 0.5, 0.6, 0.6, 0.5, 0.6, 0.5, 0.6],
            dtype=torch.float,
            device=self.device,
        )

        # self.leap_hand_default_dof_pos = (1-self.default_dof_pos_ratio) * self.leap_hand_dof_lower_limits + \
        #                                     self.default_dof_pos_ratio * self.leap_hand_dof_upper_limits
        ...
        if self.randomize_scale and self.scale_list_init:
            ...
            for s in self.randomize_scale_list:
                self.saved_grasping_states[str(s)] = (
                    torch.from_numpy(np.load(f'cache/{self.grasp_cache_name}_grasp_50k_s{str(s).replace(".", "")}.npy'))
                    .float()
                    .to(self.device)
                )
        else:
        ...
        # self.accumulate_rew_buf = torch.zeros((self.num_envs,), device=self.device, dtype=torch.float)
        # self.max_accumulate_rew = -np.inf
    ...
    def set_defaults(self):
        ...
        if "grasp_dof_search_radius" not in self.cfg["env"]:
            self.cfg["env"]["grasp_dof_search_radius"] = 0.25  # change to 0.5 for more diverse configurations
        ...
        if "random_reset_grasps" not in self.cfg["env"]:
            self.cfg["env"]["random_reset_grasps"] = True
        if self.cfg["env"]["random_reset_grasps"]:
            assert "random_object_pose_type" in self.cfg["env"] and self.cfg["env"]["random_object_pose_type"] in [
                "random_2d_z",
                "random_3d",
            ]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.leap_hand_dof_lower_limits += (2 * torch.rand_like(self.leap_hand_dof_lower_limits) - 1) * self.cfg["env"][
            "randomization"
        ]["joint_limits"]
        self.leap_hand_dof_upper_limits = self.leap_hand_dof_upper_limits.repeat((self.num_envs, 1))
        self.leap_hand_dof_upper_limits += (2 * torch.rand_like(self.leap_hand_dof_upper_limits) - 1) * self.cfg["env"][
            "randomization"
        ]["joint_limits"]
        ...
        for i in range(num_envs):
            ...
            if self.randomize_scale:
                ...
                obj_scale = np.random.uniform(
                    self.randomize_scale_list[i % num_scales] - 0.025, self.randomize_scale_list[i % num_scales] + 0.025
                )
    ...
    def reset_idx(self, env_ids):
        ...
        for n_s in range(num_scales):
            ...
            if self.cfg["env"]["random_reset_grasps"]:
                ...
                if True:
                    ...
                    # object orientation
                    if self.cfg["env"]["random_object_pose_type"] == "random_2d_z":
                        rand_floats[:3:6] *= np.pi
                        new_object_rot = quat_from_euler_xyz(
                            0.005 * rand_floats[:, 3], 0.005 * rand_floats[:, 4], rand_floats[:, 5]
                        )
                    elif self.cfg["env"]["random_object_pose_type"] == "random_3d":
                        new_object_rot = random_quaternions(num=len(s_ids), device=self.device, order="xyzw")
        ...
        if "print_object_angvel" in self.cfg["env"] and len(self.object_angvel_finite_diff_ep_buf) > 0:
            print(
                "mean object angvel: ",
                sum(self.object_angvel_finite_diff_ep_buf) / len(self.object_angvel_finite_diff_ep_buf),
            )
    ...
    def compute_observations(self):
        ...
        cur_obs_buf = (
            unscale(
                joint_noise_matrix.to(self.device) + self.leap_hand_dof_pos,
                self.leap_hand_dof_lower_limits,
                self.leap_hand_dof_upper_limits,
            )
            .clone()
        ...
        if self.cfg["env"]["include_obj_pose"]:
            cur_obs_buf = torch.cat([cur_obs_buf, self.object_pos.unsqueeze(1), self.object_rpy.unsqueeze(1)], dim=-1)
        ...
        if self.cfg["env"]["include_history"]:
            ...
            t_buf = (
                self.obs_buf_lag_history[:, -3:].reshape(self.num_envs, -1)
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], log_r_reward, olv_penalty = compute_hand_reward(
            ...
            self.angvel_clip_max,
            self.angvel_clip_min,
            pose_diff_penalty,
            pose_diff_pscale,
            torque_penalty,
            torque_pscale,
        ...
        # # compute accumulate reward
        # self.accumulate_rew_buf += self.rew_buf
        # if self.reset_buf.sum() > 0:
        #     self.max_accumulate_rew = max(torch.max(self.accumulate_rew_buf), self.max_accumulate_rew)
        # if self.max_accumulate_rew >= 0.0:
        #     save_reset = (self.accumulate_rew_buf >= 0.8 * self.max_accumulate_rew)
        #     self.reset_state_replay_buffers.save(
        #         torch.concatenate((self.init_pose_buf[save_reset], self.object_init_pose_buf[save_reset]), dim=-1),
        #     )
        # self.accumulate_rew_buf = torch.where(self.reset_buf.to(torch.bool), torch.zeros_like(self.accumulate_rew_buf), self.accumulate_rew_buf)
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                objectx = (
                    (self.object_pos[i] + quat_apply(self.object_rot[i], to_torch([1, 0, 0], device=self.device) * 0.2))
                    .cpu()
                    .numpy()
                )
                objecty = (
                    (self.object_pos[i] + quat_apply(self.object_rot[i], to_torch([0, 1, 0], device=self.device) * 0.2))
                    .cpu()
                    .numpy()
                )
                objectz = (
                    (self.object_pos[i] + quat_apply(self.object_rot[i], to_torch([0, 0, 1], device=self.device) * 0.2))
                    .cpu()
                    .numpy()
                )
    ...
    def pre_physics_step(self, actions):
        ...
        if self.force_scale > 0.0:
            ...
            obj_mass = to_torch(
                [
                    self.gym.get_actor_rigid_body_properties(env, self.gym.find_actor_handle(env, "object_cube"))[
                        0
                    ].mass
