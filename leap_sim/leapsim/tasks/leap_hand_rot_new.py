...
from leapsim.utils.torch_jit_utils import quat_diff_rad, random_quaternions, randomize_rotation_three_axis, quat_from_euler_xyz
from .buffers.reset_buffers import ResetBuffer
...
class LeapHandRotNew(VecTaskRot):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture=None, force_render=None):
        ...
        self.default_dof_pos_ratio = torch.tensor([0.5, 0.4, 0.6, 0.6, \
                                                   0.4, 0.65, 0.8, 0.85, \
                                                   0.4, 0.5, 0.6, 0.6, \
                                                   0.5, 0.6, 0.5, 0.6], dtype=torch.float, device=self.device)
    ...
    def set_defaults(self):
        ...
        if self.cfg["env"]["random_reset_grasps"]:
            assert "random_object_pose_type" in self.cfg["env"] and \
                self.cfg["env"]["random_object_pose_type"] in ["random_2d_z", "random_3d"]
            ...
            if self.cfg["env"]["random_reset_grasps"]:
                ...
                if True:
                    ...
                    if self.cfg["env"]["random_object_pose_type"] == "random_2d_z":
                        ...
                        new_object_rot = quat_from_euler_xyz(0.005*rand_floats[:, 3], 0.005*rand_floats[:, 4], rand_floats[:, 5])
