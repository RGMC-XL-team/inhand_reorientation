...
from isaacgym.torch_utils import quat_conjugate, quat_mul, to_torch, unscale, quat_apply, tensor_clamp, torch_rand_float, scale, quat_from_euler_xyz
...
class LeapHardwareAgent(object):
    """
    ...
    def set_defaults(self):
        ...
        self.ordered_fingertip_link_names = ['finger1_tip_center', 'thumb_tip_center', 'finger2_tip_center', 'finger3_tip_center']
        ...
        self.config["train"]["params"]["load_path"] = \
            os.path.join(self.package_paths['leap_sim'], 'leapsim', self.config["train"]["params"]["load_path"])
    ...
    def get_random_goal(self):
        ...
        goal_rot = quat_from_euler_xyz(torch.tensor(0.0).to(self.device), torch.tensor(0.0).to(self.device), rand_goal_rot)
        self.goal_rot[0, :] = goal_rot

        goal_pos = torch.tensor([_offset["override_object_init_x"], _offset["override_object_init_y"], _offset["override_object_init_z"]-0.5], dtype=torch.float, device=self.device)
        ...
        ros_transform = pos_quat_to_ros_transform(goal_pos.cpu().numpy(), goal_rot.cpu().numpy()[[-1, 0, 1, 2]], child_frame="cube_goal")
    ...
    def compute_full_observation_hw(self, no_vel=False):
        ...
        if self.override_object_z:
            object_pose_override[0, 2] = self.config["task"]["env"]["override_dist"]["cube_small"]["override_object_init_z"]
    ...
    def deploy(self):
        ...
        self.move_hand_to_pose(
            leap_start_positions=self.leap_hand_dof_pos.squeeze().cpu().numpy(),
            leap_end_positions=self.init_pose
        )
        import pdb; pdb.set_trace()
...
if __name__ == "__main__":
    with open("/home/yongpeng/competition/RGMC_XL/leap_ws/src/leap_sim/leapsim/cfg/dict/leaphand_rot_z_goal.yaml", "r") as f:
